...
| 迁移 | Alembic |
| 模板与前端 | Jinja2, 本地 Tailwind Play 运行时, Alpine.js |
...
`AUTO_MIGRATE_SQLITE` 默认保持 `false`；启动时如果检测到已有 SQLite 数据库的 Alembic 版本落后于当前代码，应用会拒绝启动并提示执行迁移，避免在页面访问时才出现 500。自部署单实例 SQLite 可在确认备份策略后显式设置 `AUTO_MIGRATE_SQLITE=true` 自动迁移。
...
- 程序化调用：`X-API-Key: {api_key}`，API Key scope 支持 `send` 与 `read_inbound`
- Web 表单：生产环境启用 CSRF token 校验

新建 API Key 默认授予 `send,read_inbound`，保持程序化收发邮件的默认能力；也可以在 Web UI 或 API 中只勾选所需 scope。`send` 可调用 `/api/v1/outbound/send`，`read_inbound` 可读取 `/api/v1/inbound` 与 `/api/v1/inbound/{id}`，资源管理接口不开放给 API Key。

限流默认使用直连 IP，不信任客户端可伪造的 `X-Forwarded-For`。如果部署在可信反向代理后并需要真实客户端 IP，请同时设置 `TRUST_PROXY_HEADERS=true` 和 `TRUSTED_PROXY_IPS`，例如 `TRUSTED_PROXY_IPS=127.0.0.1,10.0.0.0/8`。
